...
| Linux installers + binary tarball (deb/rpm → `dist/`) | `./scripts/linux.sh` |
...
- **Logging.** Filter precedence: `BILI_LOG` (app-specific) > `RUST_LOG` > built-in default `info`. Per-segment proxy lines (`stream::proxy`) and dash.js per-fragment events (`FRAG_*`) are emitted at DEBUG and stay quiet under the default — bump to `BILI_LOG=bili_rust_lib=debug` to bring them back, or `BILI_LOG=bili_rust_lib=trace` when diagnosing WBI/sign failures. Both vars use the standard `tracing_subscriber::EnvFilter` directives syntax.
